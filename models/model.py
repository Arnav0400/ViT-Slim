...
@register_model
def deit_small_patch16_224(pretrained=False, head_search=False, uniform_search=False, **kwargs):
    ModuleInjection.method = kwargs.pop('method', 'full')
    ModuleInjection.searchable_modules = []
    model = VisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), head_search=head_search, uniform_search=uniform_search, **kwargs)
    model.searchable_modules = ModuleInjection.searchable_modules
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
